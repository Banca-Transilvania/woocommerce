...
if ( $this->can_show_form( $payment_data ) ) {
	?>
	<h4><?php echo esc_html__( 'Capture or cancel payment:' ); ?></h4>
	...
	<div>
		<label for="bt-ipay-capture-amount">
			<?php echo esc_html__( 'Capture amount:' ); ?>
		</label>
	</div>
	...
	<button type="button" class="button button-primary bt-ipay-ajax-capture" type="button">
		<?php echo esc_html__( 'Capture', 'bt-iPay' ); ?>
	</button>
	<button type="button" class="button button-primary bt-ipay-ajax-cancel" type="button">
		<?php echo esc_html__( 'Cancel', 'bt-iPay' ); ?>
	</button>
	<hr style="margin-top:20px;">
	<?php
}
?>
<h4><?php echo esc_html__( 'Payment history:' ); ?></h4>
<table class="wp-list-table widefat fixed table-view-list">
	<thead>
		<tr>
			<th scope="col">
				<?php esc_html_e( 'Payment id', 'bt-ipay' ); ?>
			</th>
			
			<th scope="col" style="text-align:right;">
				<?php esc_html_e( 'Amount', 'bt-ipay' ); ?>
			</th>

			<th scope="col" style="text-align:right;">
				<?php esc_html_e( 'Status', 'bt-ipay' ); ?>
			</th>
		</tr>
	</thead>
	<tbody>
		...
		foreach ( $payments as $payment ) {
			...
			if ( isset( $payment['loy_id'] ) && strlen( (string) $payment['loy_id'] ) > 0 ) {
				...
				<tr>
					<td>
						<?php echo esc_html__( 'LOY: ', 'bt-ipay' ); ?>
